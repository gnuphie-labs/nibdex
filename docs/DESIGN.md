...
- `recent_commits(filter?: string, days?: int, repo?: string, limit?: int)` → digest of git commit entries by recency-weighted relevance, with metadata: commit_hash (short), authored_at, author_email, repo_path, message_summary, files_changed, parent_hashes. Optional `filter` runs against message text or files_changed paths; optional `repo` scopes to one nested repository.
- `find_commit(query: string, repo?: string, limit?: int)` → ranked commit entries by content match against `message_summary || message_body || files_changed` (lexical, not semantic). Returns structured commit record with summary, body, files changed, and cross-refs to related session entries that mention the same files.
